...
def _(A, QA, RA, mo):
    _v1_stack = mo.vstack(
        [mo.md("#### **Original Vectors (A)**"), mo.md(to_latex(A))], align="center"
    )

    _v2_stack = mo.vstack(
        [mo.md("#### **Orthonormal (Q)**"), mo.md(to_latex(QA.astype("int64")))], align="center"
    )

    _v3_stack = mo.vstack(
        [mo.md("#### **Upper Triangular (R)**"), mo.md(to_latex(RA.astype("int64")))],
        align="center",
    )

    stack = mo.hstack(
        [_v1_stack, mo.md("## **QR Decomposition** ➡️").center(), _v2_stack, _v3_stack],
        align="center",
        gap=0,
        widths=[0.3, 0.5, 0.20, 0.30],
    )
...
def _(fig, mo):
    ...
    return bullet_pts, desc_md, orientation_md, plot, sidenote


@app.cell
def _(bullet_pts, desc_md, mo, orientation_md, plot, sidenote):
    # stacking
